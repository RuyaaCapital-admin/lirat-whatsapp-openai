...
function windowFor(scope: string) {
  ...
  if (scope === "last") { start.setDate(now.getDate() - 3); }       // last 72h
  else if (scope === "today") { start.setDate(now.getDate() - 0); end.setDate(now.getDate() + 1); }
  else { /* next */ end.setDate(now.getDate() + 7); }               // next 7d
  return { d1: ymd(start), d2: ymd(end) };
}

function regionsForSymbol(sym?: string): string[] {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const countries = (req.query.region as string)?.split(",").map(s=>s.trim()).filter(Boolean)
    ...
    // Map to top 3 high-impact items
    ...
    // Return both structured + ready text lines
